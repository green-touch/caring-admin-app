...
import { useLoginToggle } from '@_hooks/login/useLoginToggle';
...
const LoginScreen = ({ navigation }: LoginScreenProps) => {
    ...
    const { 회원번호, 비밀번호, handle회원번호, handle비밀번호, isInputValid } = useLoginInput();
    ...
    return (
        <SafeAreaView className="flex flex-1 w-full px-8 bg-white items-center justify-start">
            ...
            <LoginButton buttonTitle='로그인' onPress={handleLoginPress} disabled={isInputValid} />
            <LoginHelp />
        </SafeAreaView>
    )
}
